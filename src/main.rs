...
fn transcoder_thread(path: &str) {
    let filename = Path::new(path).file_name().unwrap().to_str().unwrap();
    let outfile = format!("/Users/khn/{}_transcoded.wav", filename);
...
fn consumer_thread(rx: std::sync::mpsc::Receiver<PathBuf>) {
    loop {
        if let Ok(path) = rx.recv() {
            println!("Processing file: {:?}", path);
            transcoder_thread(path.to_str().unwrap());
